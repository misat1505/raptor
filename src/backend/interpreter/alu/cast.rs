...
impl ALU {
    pub(in crate::backend::interpreter) fn cast_to_type(val: Value, to_type: &Type, span: Span) -> Result<Value, ComputationError> {
        match (val, to_type) {
            ...
            (Value::Bool(bool), Type::Str) => Ok(Value::String(String::from(if bool { "true" } else { "false" }))),

            (Value::Bool(bool), Type::I64) => Ok(Value::I64(if bool { 1 } else { 0 })),

            (Value::Bool(bool), Type::F64) => Ok(Value::F64(if bool { 1.0 } else { 0.0 })),
