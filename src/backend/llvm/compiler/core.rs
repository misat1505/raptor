...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    ...
    pub fn optimize(&self, level: OptimizationLevel) -> Result<(), Box<dyn IError>> {
        ...
        let target = Target::from_triple(&triple).map_err(|err| {
            Box::new(CompilerError::at(
                ErrorSeverity::HIGH,
                format!("Failed to get target: {}", err),
                self.span,
            )) as Box<dyn IError>
        })?;
        ...
        let target_machine = target
            ...
            .ok_or_else(|| {
                Box::new(CompilerError::at(
                    ErrorSeverity::HIGH,
                    String::from("Failed to create target machine"),
                    self.span,
                )) as Box<dyn IError>
            })?;
        ...
        self.module
            ...
            .map_err(|err| {
                Box::new(CompilerError::at(
                    ErrorSeverity::HIGH,
                    format!("Module optimization failed: {}", err),
                    self.span,
                )) as Box<dyn IError>
            })?;
    ...
    pub fn write_ir_to_file(&self, path: &str) -> Result<(), Box<dyn IError>> {
        self.module.print_to_file(path).map_err(|err| {
            Box::new(CompilerError::at(
                ErrorSeverity::HIGH,
                format!("Failed to write IR to file: {}", err),
                self.span,
            )) as Box<dyn IError>
        })
    }
