...
fn setup_alu(policy: OverflowPolicy) -> (Context, LlvmAlu, Span) {
    let context = Context::create();
    let alu = LlvmAlu::new(policy);
...
fn cast_same_type_is_identity() {
    let (context, alu, span) = setup_alu(OverflowPolicy::Warn);
    ...
    for (val, ty) in cases {
        let result = alu.cast_to_type(&builder, &libc, val, &ty, span).unwrap();
...
fn cast_i8_to_wider_signed() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::I16, span).unwrap(),
        LlvmValue::I16(_)
    ));

    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::I32, span).unwrap(),
        LlvmValue::I32(_)
    ));

    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::I64, span).unwrap(),
        LlvmValue::I64(_)
    ));
}
...
fn cast_integer_to_f64() {
    ...
    for v in signed {
        assert!(matches!(
            alu.cast_to_type(&builder, &libc, v, &Type::F64, span).unwrap(),
            LlvmValue::F64(_)
        ));
    }
...
fn cast_f64_to_integers() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::I8, span).unwrap(),
        LlvmValue::I8(_)
    ));
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::U8, span).unwrap(),
        LlvmValue::U8(_)
    ));

    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::U64, span).unwrap(),
        LlvmValue::U64(_)
    ));
}
...
fn cast_integer_to_bool() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v, &Type::Bool, span).unwrap(),
        LlvmValue::Bool(_)
    ));
...
fn cast_bool_to_integers_and_f64() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::U32, span).unwrap(),
        LlvmValue::U32(_) | LlvmValue::I32(_)
    ));

    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::F64, span).unwrap(),
        LlvmValue::F64(_)
    ));
}
...
fn cast_bool_to_str() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v, &Type::Str, span).unwrap(),
        LlvmValue::Str(_)
    ));
}
...
fn cast_char_u8_roundtrip() {
    ...
    let as_u8 = alu.cast_to_type(&builder, &libc, ch, &Type::U8, span).unwrap();

    assert!(matches!(as_u8, LlvmValue::U8(_)));

    let back = alu.cast_to_type(&builder, &libc, as_u8, &Type::Char, span).unwrap();
...
fn cast_unsupported_fails() {
    ...
    for (val, ty) in bad {
        let err = alu.cast_to_type(&builder, &libc, val, &ty, span).unwrap_err();
...
fn cast_i64_to_narrower() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v, &Type::I8, span).unwrap(),
...
fn cast_signed_to_unsigned() {
    ...
    assert!(matches!(alu.cast_to_type(&builder, &libc, v, &Type::U8, span).unwrap(), LlvmValue::U8(_)));
}
...
fn cast_unsigned_to_signed_and_wider() {
    ...
    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v.clone(), &Type::U16, span).unwrap(),
        LlvmValue::U16(_)
    ));

    assert!(matches!(
        alu.cast_to_type(&builder, &libc, v, &Type::I64, span).unwrap(),
